...
(*PrinterFunctions*)
let rec print_tc (tc:TypeCtxt.t) : unit =
  print_endline ("Type Ctxt");
  print_endline "=========================================";
  print_endline "{";
  print_id_fields tc;
  print_endline "}";
...
and print_id_fields (tc:TypeCtxt.t) : unit =
  match tc with 
    |[] -> ()  
    |(id,fields)::rest -> print_endline ("Struct "^id^": "); print_fields fields; print_id_fields rest;

and print_fields (fields:Ast.field list) : unit =
  match fields with 
    |[] -> ()  
    |f::fs -> print_endline (Astlib.ml_string_of_field f); print_fields fs

let print_exp_tripple (ty,op,str:(Ll.ty * Ll.operand * stream)) : unit = 
  print_endline ("expression tripple looks like :"^(Llutil.soop (ty,op)))


let rec print_ctxt (c:Ctxt.t) : unit =
  print_endline "Ctxt:";
  print_endline "---------------------------------------";
  print_id_ty_op c; 
  print_endline "---------------------------------------"

and print_id_ty_op (c:Ctxt.t) : unit =
  match c with
    |[] -> ()
    |(id, tyop)::rest -> print_endline (id^": "^(Llutil.soop tyop)); print_id_ty_op rest

let rec print_stream : stream -> unit = function
  |[] -> ()
  |i::is -> print_I i; print_stream is

and print_I : elt -> unit = function
  |I (uid,insn) -> print_endline (Llutil.string_of_named_insn (uid,insn) )
  |_ -> print_endline "printing non I instruction in print_I"
...
let get_struct_size ct (id:Ast.id) : Ll.operand =                    (*returns size of struct as Const Int64 *)
  let flist = TypeCtxt.lookup id ct in
  (Const (Int64.of_int ((List.length flist) * 8)))                                         (*all oat types are represented as 8Byte pointers*)

let oat_alloc_struct ct (id:Ast.id) : Ll.ty * operand * stream =
  let (ans_id, struct_id) = gensym "struct", gensym "raw_struct" in 
  let ans_ty = cmp_ty ct @@ TRef (RStruct id) in  (*Ptr (Namedt id)*)
  let struct_ty = Ptr I64 in                      (*oat malloc returns Ptr(i64) which we will then bitcast to our ansty*)
  let size = get_struct_size ct id in
  (ans_ty, Id ans_id, lift [
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length e ->
    ...
    let gep_id, length_id = gensym "gep", gensym "length" in
    let code = code >@ [I (gep_id, Gep (ty,op,[Const 0L;Const 0L]))] in
    let code = code >@ [I (length_id, Load (Ptr I64, Id gep_id))] in
    (I64, Id length_id, code)
  ...
  *)
  ...
  | Ast.NewArr (elt_ty, e1, id, e2) ->
      ...
      let (arr_ty, arr_op, alloc_code) = oat_alloc_array tc elt_ty size_op in
      (*New array is allocated and stored in a {I64, [ 0x TYPE ]}* bitcast should not be necessary because elt_ty gets passed as arg*)
    
      let ast_array_id = gensym "temp_array" in
      let temp_array_id = gensym "new_array_ptr" in
      let array_ctxt_code = lift [
        (temp_array_id, Alloca (arr_ty));
        ("", Store (arr_ty, arr_op , Id temp_array_id ))
        ] in 
      let c = Ctxt.add c ast_array_id (Ptr arr_ty, Ll.Id temp_array_id ) in
      

      let ast_length_id = gensym "loop_length" in
      let temp_length_id = gensym "length_ptr" in
      let length_ctxt_code = lift [
        (temp_length_id, Alloca (I64));
        ("", Store (I64,size_op,Id temp_length_id))
      ] in
      let c = Ctxt.add c ast_length_id (Ptr size_ty,Ll.Id temp_length_id) in


      (*| For of vdecl list * exp node option * stmt node option * stmt node list*)
      let var_init =  [(id, no_loc @@ CInt 0L)] in
      let bool_exp = (Some (no_loc (Bop (Lt, no_loc (Id id), no_loc (Id ast_length_id))))) in (*!!! HARDCODED 3 FOR ARRAY LENGTH*)
      let increment_stmt = Some ( no_loc  (Assn (no_loc (Id id), no_loc (Bop (Add, no_loc (Id id), no_loc (CInt 1L)))))) in 
      let Id arr_id = arr_op in
      let loop_body =  [no_loc @@ Assn (no_loc @@ Index (no_loc (Id ast_array_id) , no_loc (Id id)), e2)] in
      let for_stmt = For (var_init, bool_exp, increment_stmt, loop_body) in 
      let (new_c,for_code) = cmp_stmt tc c Ll.Void (no_loc for_stmt)  in 
  
  
      
  
      (arr_ty, arr_op, size_code >@ alloc_code >@ array_ctxt_code >@ length_ctxt_code >@ for_code)
   ...
   *)
  | Ast.CStruct (struct_id, l) ->
    
    let (struct_ty,struct_op,alloc_code) = oat_alloc_struct tc struct_id in  (*structop now holds a pointer to the location of the first field*)
    let store_fields_code = store_elements_in_struct tc c struct_op struct_ty 0 struct_id l in
    (struct_ty, struct_op, alloc_code >@ store_fields_code)
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (e, fid) ->
    ...
    (* let (struct_ty,struct_op,lhs_code) = cmp_exp_lhs tc c e in *)
    let (struct_ty2, struct_op2,rhs_code) = cmp_exp tc c e in
    

    begin match struct_ty2 with
    |Ptr(Ptr(Namedt sname)) ->
      let Ptr (Ptr(Namedt sname)) = struct_ty2 in
      let (fieldty, fieldind) = TypeCtxt.lookup_field_name sname fid tc in 
      let ll_field_ty = cmp_ty tc fieldty in
      let load_id = gensym ("loadstructptr"^sname) in
      let gep_id = gensym ("gep_"^sname^"_"^fid) in

      (* let code = lhs_code >@ [I (load_id, Load (struct_ty,struct_op))] in *)

      let code = rhs_code >@ [I (gep_id, Gep (Ptr(Namedt sname), struct_op2,[Const 0L;Const fieldind]))] in 
      (ll_field_ty,Id gep_id,code)



    |Ptr (Namedt sname) -> 
      let Ptr(Namedt sname) = struct_ty2 in
      let (fieldty, fieldind) = TypeCtxt.lookup_field_name sname fid tc in 
      let ll_field_ty = cmp_ty tc fieldty in
      let load_id = gensym ("loadstructptr"^sname) in
      let gep_id = gensym ("gep_"^sname^"_"^fid) in
  ...
  *)
  | Ast.Index (e, i) ->
    ...
    let code = [] in
    (* Lars Todo:
       - call oat assert arraylength on the right arguments 
       - throw exception (or type-error?) if index out of bounds
       - bitcast ArrayType to right length
       *)
  ...
  | _ -> failwith ("invalid lhs expression"^(Astlib.string_of_exp e))


(*
    Lars: In this function we're filling a struct with pointers to its fields, the assumptions are:
   - structptr contains P* for some Struct P
   - the struct has previously been malloced to have 8 Bytes of memory for each field
   - we can access structptr as follows -> gep [0,i] on P* -> gep [i] on P -> address of ith field in P 
   *)
and store_elements_in_struct tc c (structptr: Ll.operand) (struct_ty:Ll.ty) (index:int) (structid:Ast.id) (flist:(Ast.id * Ast.exp node) list) : stream =     
    match flist with
      |[] -> []
      |(fid,exp)::fields -> 
        let (init_expty,expop,expstr) = cmp_exp tc c exp in
        let expty = match init_expty with
          |Ptr (Struct [l; Array (n,t)]) -> Ptr(Struct [l; Array(0,t)])
          |_-> init_expty
        in
        let clean_op_id = gensym "bitcast_for_arrays" in
        let code = expstr >@ [I (clean_op_id , Bitcast (init_expty, expop, expty))] in
        let expop = Ll.Id clean_op_id in (*shadowing expop so i dont have to change rest of program*)
        let raw_id, clean_id ,gep_id = gensym ("raw_"^fid) , gensym ("clean_"^fid), gensym ("gep_"^fid) in
        let code = code >@ [ I (raw_id, Call (Ptr(I64), Gid "oat_malloc", [I64,Const 8L])) ] in
        let code = code >@ [I (clean_id, Bitcast (Ptr(I64),Id raw_id,Ptr(expty)))] in 
        let code = code >@ [I (gep_id, Gep (struct_ty,structptr,[Const 0L;Const (Int64.of_int index)]))] in
        let code = code >@ [I ("", Store (expty, expop , Ll.Id gep_id))] in 
        code >@ store_elements_in_struct tc c structptr struct_ty (index + 1) structid fields 
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    ...
    let c = Ctxt.add c id (Ptr (cmp_ty tc (TRef typ)), Gid id) in
    let nullcheck_id = gensym "nullcheck" in
    let exp_code = exp_code >:: I (nullcheck_id, Icmp (Eq, exp_ty, exp_op, Null)) in
    c, exp_code
        >:: T (Cbr (Ll.Id nullcheck_id, lnotnull, lnull))
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (sname, id_expnd_list) ->
    let expnds =  (List.map snd id_expnd_list) in
    let elts, gs = List.fold_right
        (fun cst (elts, gs) ->
           let gd, gs' = cmp_gexp c tc cst in
           (gd::elts, gs' @ gs)   ) expnds ([], [])
    in

    
    let struct_ty = Namedt sname in
...
let cmp_prog (p:Ast.prog) : Ll.prog =
  ...
  { tdecls = tctxt_to_tdecls tc; gdecls; fdecls; edecls }
   ...
   (*
    - typecheck tdecl still in create_struct_ctxt
    - create ctxt doesn't check for duplicates yet
    - check global expressions dont contain global variables
